...
local ball = display.newImage ("pequena.png",_W/2, 400)
physics.addBody(ball, {density = 1, friction = 0.1, bounce =0.2})
screenGroup:insert(ball)
ball.isBullet=true
local ob1 = display.newRect (0, 250, 200, 20)
physics.addBody (ob1, "static", {bounce=1})
screenGroup:insert(ob1)

physics.setGravity(0,0)
system.activate( "multitouch" )
local function dragBody( event )
	return gameUI.gameUI.dragBody( event, { maxForce=500} )
...
function  handleButtonEvent( event )
	...
	storyboard.gotoScene( "segundafase", "slideLeft", 1100  )
	-- body
end
