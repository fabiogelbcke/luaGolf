...
physics.start()
local bg = display.newImage ("moonbg.jpg", true)
...
vex=math.random(-300,300)
parameter = 300 - math.abs(vex)
vey=math.random(parameter,300)
hole:setLinearVelocity(vex,vey)
...
local function ballFriction()
    ballImage.x=ball.x
    ballImage.y=ball.y
    vx,vy = hole:getLinearVelocity()
    if (hole.x <= 40 or hole.x >= 280)then
        hole:setLinearVelocity(-vx,vy)
    end
    if hole.y<=40 or hole.y>=380 then
        hole:setLinearVelocity(vx,-vy)
    end
    ...
    if (vx ~=0 or vy ~=0)then
        ...
        local newvx=vx*(1 - (500)/((vx*vx)+(vy*vy)))
        local newvy=vy*(1 - (500)/((vx*vx)+(vy*vy)))
...
function scene:enterScene( event )
    ...
    storyboard.purgeScene( "sextafase" )
end
